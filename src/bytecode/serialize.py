...
def serialize_instruction(block_writer, instruction):
    ...
    elif isinstance(instruction, bytecode.ConstantChar):
        block_writer.constant_char(instruction.value)
    elif isinstance(instruction, bytecode.ConstantByteString):
    ...
    elif isinstance(instruction, bytecode.ConstantDouble):
        block_writer.constant_double(instruction.value)
...
def serialize_terminator(block_writer, terminator):
    ...
    elif isinstance(terminator, bytecode.Throw):
        block_writer.throw(terminator.exception)
    else:
